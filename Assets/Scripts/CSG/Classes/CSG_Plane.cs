...
namespace Assets.Scripts.CSG.Classes
{
	/**
	...
	class CSG_Plane
	{
		...
		public void SplitPolygon(CSG_Polygon polygon, List<CSG_Polygon> coplanarFront, List<CSG_Polygon> coplanarBack, List<CSG_Polygon> front, List<CSG_Polygon> back)
		{
			...
			for (int i = 0; i < polygon.vertices.Count; i++) 
			{
				...
				EPolygonType type = (t < -Assets.Scripts.CSG.CSG.EPSILON) ? EPolygonType.Back : ((t > Assets.Scripts.CSG.CSG.EPSILON) ? EPolygonType.Front : EPolygonType.Coplanar);
				polygonType |= type;
				types.Add(type);
			}
