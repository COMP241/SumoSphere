...
{
    // This triangulator taken from http://wiki.unity3d.com/index.php?title=Triangulator
    ...
    public static Mesh PolygonExtrude(Vector2[] poly, Vector3 offset, float height = 1f)
    {
        ...
        {
            vertices[i] = new Vector3(poly[i].x, 0, poly[i].y) + offset;
            vertices[i + poly.Length] = new Vector3(poly[i].x, height, poly[i].y) + offset;
        }
        ...
        m.SetUVs(0, vertices.Select(v => Vector2.zero).ToList());
